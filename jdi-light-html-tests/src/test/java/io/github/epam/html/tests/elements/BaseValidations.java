...
public class BaseValidations {
    ...
    public static void baseValidation(ICoreElement el) {
        // EXCLUDED el.hover();
        webSettings.logger.info("Check enabled");
        assertTrue(el.isEnabled());
        webSettings.logger.info("Check displayed");
        assertTrue(el.isDisplayed());
        webSettings.logger.info("Check disabled");
        assertFalse(el.isDisabled());
        webSettings.logger.info("Check hidden");
        assertFalse(el.isHidden());
        webSettings.logger.info("Check getLocation");
        Point location = el.core().getLocation();
        assertTrue(location.x > 0 && location.y > 0, "Location: " + location);
        webSettings.logger.info("Check getSize");
        ...
        webSettings.logger.info("Check setAttribute");
        el.setAttribute("test-jdi", "test-value");
        assertEquals(el.attr("test-jdi"), "test-value");
        webSettings.logger.info("Check highlight");
        el.highlight("blue");
        el.highlight();
        webSettings.logger.info("Check show");
        el.show();
    }

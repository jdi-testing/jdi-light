 ...
 */
public interface IDataGrid<L extends PageObject, D> extends HasGrid, ICoreElement {
    ...
    default List<D> allData() {
        return map(grid().rows(), row -> rowAsData(row.uiElements()));
    }
    ...
    default List<L> lines() {
        return map(grid().rows(), row -> rowAsLine(row.uiElements()));
    }
    // endregion
    ...
    default List<D> datas(Matcher<D> matcher) { return filter(allData(), matcher::matches); }

...
public class DataListTemp<T extends IListBase, D> extends ListBase<T, DataListAssert<T, D>> {

    private CacheValue<MapArray<String, T>> map = new CacheValue<>(MapArray::new);
    public JList<T> list() {
        return new JList<>(core().getLocator()).setup(JList.class, e-> e.core()
            .setName(getName() + " list")
            .setParent(core().parent));
    }
    public Class<D> dataType;

    public DataListTemp() {}
    public DataListTemp(Class<T> type) { initClass = type; }
    ...
    protected MapArray<String, T> getMap(int minAmount) {
        if (actualMapValue())
            return map.get();
        List<T> els = elements(minAmount);
        return new MapArray<>(els.size(),
                i -> elementTitle(els.get(i).core().get()),
                els::get);
    }

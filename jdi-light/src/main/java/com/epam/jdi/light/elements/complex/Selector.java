...
public class Selector extends UIBaseElement<UISelectAssert>
    ...
    public void select(String value) {
        WebList list = this.finds("//li");
        if ("firefox".equals(core().driverName) && list.size() > 0) {
            if (list.get(0).isHidden()) {
                core().click();
            }
            list.foreach(li -> {
                if (value.equals(li.getText())) li.click();
            });
        } else {
    ...
    public void check(String... values) {
        ...
        if ("firefox".equals(core().driverName) && list.size() > 0) {
            ...
            list.stream().filter(l -> asList(values).contains(l.getText())).forEach(l -> l.click());
        } else {
    ...
    public void unckeckAll() {
        ...
        if ("firefox".equals(core().driverName) && list.size() > 0) {
            ...
            List<String> selectedValues = asSelect().getOptions().stream().filter(o -> o.isSelected()).map(o -> o.getText()).collect(Collectors.toList());
            list.stream().filter(l -> selectedValues.contains(l.getText())).forEach(l -> l.click());
        } else {

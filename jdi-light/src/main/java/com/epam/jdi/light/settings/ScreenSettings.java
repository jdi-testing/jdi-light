...
public class ScreenSettings {
    public String screenPath = mergePath(COMMON.logsPath, "screens");
    public String screenFileSuffix = "jpg";
    public AttachmentStrategy screenshotStrategy = ON_FAIL;
    ...
    public JFunc1<String, String> getScreenName = screenName -> {
        ...
        String newName;
        do {
            newName = format("%s_%s.%s", fileName, num++, screenFileSuffix);
        } while (new File(newName).exists());
        return newName;
    };
    private String getPath() {
        if (isBlank((screenPath))) return "";
        String result = screenPath.replace("/", "\\");
        return result.contains(":")
            ? screenPath
            : mergePath(path(""), screenPath);
    }
}

...
import com.epam.jdi.mobile.common.FormFilters;
...
import com.epam.jdi.mobile.elements.pageobjects.annotations.Mandatory;
...
import static com.epam.jdi.mobile.common.FormFilters.*;
import static com.epam.jdi.mobile.common.UIUtils.GET_BUTTON;
import static com.epam.jdi.mobile.common.UIUtils.getMapFromObject;
...
public class Form<T> extends Section {
    ...
    public void fillAction(Field field, Object element, Object parent, String setValue) {
        WebSettings.logger.debug("Fill element '%s' with value '%s'", getFieldName(field, element), setValue);
        FILL_ACTION.execute(field, element, parent, setValue);
    }
    public String getAction(Field field, Object element, Object parent) {
        WebSettings.logger.debug("Try to get element '%s' value", getFieldName(field, element));
    ...
    public void fill(MapArray<String, String> map) {
        ...
        for (Pair<String, String> pair : map)
            try {
                setField = first(allFields, f -> namesEqual(pair.key, WebAnnotationsUtil.getElementName(f)));
                if (setField == null)
                    continue;
        ...
        switch (getFilter()) {
            case MANDATORY:
                return LinqUtils.where(getFields(obj, SetValue.class),
                        field -> WebAnnotationsUtil.hasAnnotation(field, Mandatory.class));
            case OPTIONAL:
                return LinqUtils.where(getFields(obj, SetValue.class),
                        field -> !WebAnnotationsUtil.hasAnnotation(field, Mandatory.class));
            default:
    ...
    protected List<String> verify(MapArray<String, String> map) {
        ...
        for (Field field : allFields()) {
            String fieldValue = map.firstValue((name, value) ->
                    namesEqual(name, WebAnnotationsUtil.getElementName(field)));

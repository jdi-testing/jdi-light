...
import com.epam.jdi.mobile.elements.interfaces.base.HasCache;
...
import com.epam.jdi.mobile.elements.init.InitActions;
import com.epam.jdi.mobile.elements.init.UIFactory;
import com.epam.jdi.mobile.settings.TimeoutSettings;
...
import static com.epam.jdi.mobile.driver.WebDriverByUtils.*;
import static com.epam.jdi.mobile.elements.init.UIFactory.$$;
import static com.epam.jdi.mobile.logger.LogLevels.*;
...
public abstract class JDIBase extends DriverBase implements IBaseElement, HasCache {
    ...
    public JDIBase() {
        searchRules.add(WebSettings.SEARCH_RULES);
    }
    ...
    public WebElement beforeSearch(WebElement el) {
        (beforeSearch == null ? WebSettings.BEFORE_SEARCH : beforeSearch).execute(new UIElement(el));
        return el;
    }
    ...
    public JDIBase noValidation() {
        return setSearchRule(WebSettings.ANY_ELEMENT);
    }
    ...
    public JDIBase searchVisible() {
        return setSearchRule(WebSettings.VISIBLE_ELEMENT);
    }
    public JDIBase visibleEnabled() { return setSearchRule(WebSettings.ENABLED_ELEMENT); }
    public JDIBase inView() {
        showBefore();
        return setSearchRule(WebSettings.ELEMENT_IN_VIEW);
    }
    ...
    public int getTimeout() {
        return timeout > -1 ? timeout : TimeoutSettings.TIMEOUT.get();
    }
    ...
    private WebElement getSmart() {
        try {
            WebElement element = WebSettings.SMART_SEARCH.execute(this);
            if (element != null)
                return element;
            throw exception("");
        } catch (Exception ex) {
    ...
    public List<WebElement> getAllElements(Object... args) {
        ...
        if (locator.isEmpty())
            return singletonList(beforeSearch(WebSettings.SMART_SEARCH.execute(this)));
    ...
    public WebList list(Object... args) {
        return UIFactory.$$(getAll(args), getName());
    }
    ...
    public void dropToGlobalTimeout() {
        waitSec(TimeoutSettings.TIMEOUT.get());
    }
    ...
    private SearchContext getSearchContext(Object element) {
        ...
        return locator != null
            ...
            : InitActions.isPageObject(element.getClass())
                ? searchContext
                : WebSettings.SMART_SEARCH.execute(bElement.waitSec(getTimeout()));
    }
    ...
    public static JFunc1<JDIBase, String> PRINT_ELEMENT = element -> {
        ...
        return Switch(WebSettings.logger.getLogLevel()).get(
                Case(l -> l == STEP,
                        l -> StringUtils.msgFormat(OutputTemplates.PRINT_ELEMENT_STEP, element)),
                Case(l -> l == INFO,
                        l -> StringUtils.msgFormat(OutputTemplates.PRINT_ELEMENT_INFO, element)),
                Case(l -> l == ERROR,
                        l -> StringUtils.msgFormat(OutputTemplates.PRINT_ERROR_STEP, element)),
                Default(l -> StringUtils.msgFormat(OutputTemplates.PRINT_ELEMENT_DEBUG, element))
        );
    };
    ...
    public ElementArea clickAreaType = WebSettings.CLICK_TYPE;
    public TextTypes textType = WebSettings.TEXT_TYPE;
    public SetTextTypes setTextType = WebSettings.SET_TEXT_TYPE;

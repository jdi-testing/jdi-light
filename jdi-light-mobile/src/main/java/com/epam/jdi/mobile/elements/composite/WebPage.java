...
import com.epam.jdi.mobile.elements.pageobjects.annotations.WebAnnotationsUtil;
...
import static com.epam.jdi.mobile.common.CheckTypes.*;
import static com.epam.jdi.mobile.common.Exceptions.exception;
import static com.epam.jdi.mobile.common.PageChecks.EVERY_PAGE;
import static com.epam.jdi.mobile.common.PageChecks.NEW_PAGE;
import static com.epam.jdi.mobile.common.VisualCheckPage.CHECK_NEW_PAGE;
import static com.epam.jdi.mobile.common.VisualCheckPage.CHECK_PAGE;
import static com.epam.jdi.mobile.driver.ScreenshotMaker.getPath;
import static com.epam.jdi.mobile.driver.WebDriverFactory.*;
import static com.epam.jdi.mobile.elements.base.OutputTemplates.*;
import static com.epam.jdi.mobile.elements.init.PageFactory.*;
...
public class WebPage extends DriverBase implements PageObject {
    ...
    void setUrl(String uri, String template, CheckTypes validate) {
        ...
        if (!uri.contains("://"))
            url = WebAnnotationsUtil.getUrlFromUri(uri);
        else  { if (isBlank(uri)) url = WebSettings.getDomain(); }
    }
    public void updatePageData(Url urlAnnotation, Title titleAnnotation) {
        ...
        else setUrl(WebSettings.getDomain());
    ...
    public static class StringCheckType {
        ...
        public boolean check() {
            ...
            WebSettings.logger.toLog(format("Check that page %s(%s) equals to '%s'", what, value, equals));
            return equals == null
                    || equals.equals("")
                    || value.equals(equals);
        }
        ...
        public boolean match() {
            String value = actual.get();
            WebSettings.logger.toLog(format("Check that page %s(%s) matches to '%s'", what, value, equals));
            return equals == null
                    || equals.equals("")
        ...
        public boolean contains() {
            String value = actual.get();
            WebSettings.logger.toLog(format("Check that page %s(%s) contains '%s'", what, value, equals));
    ...
    public static JAction1<WebPage> BEFORE_NEW_PAGE = page -> {
        ...
        if (WebSettings.VISUAL_PAGE_STRATEGY == CHECK_NEW_PAGE)
            visualWindowCheck();
        WebSettings.logger.toLog("Page: " + page.getName());
        TimeoutSettings.TIMEOUT.set(TimeoutSettings.PAGE_TIMEOUT.get());
    };

package com.epam.jdi.mobile.actions;

import com.epam.jdi.mobile.asserts.generic.JAssert;
import com.epam.jdi.mobile.common.JDIAction;
import com.epam.jdi.mobile.common.PageChecks;
import com.epam.jdi.mobile.common.VisualCheckPage;
...
import com.epam.jdi.mobile.elements.pageobjects.annotations.VisualCheck;
import com.epam.jdi.mobile.logger.AllureLoggerHelper;
import com.epam.jdi.mobile.logger.LogLevels;
import com.epam.jdi.mobile.elements.base.OutputTemplates;
import com.epam.jdi.mobile.elements.common.WindowsManager;
...
import static com.epam.jdi.mobile.common.VisualCheckAction.ON_VISUAL_ACTION;
import static com.epam.jdi.mobile.driver.ScreenshotMaker.takeScreenOnFailure;
import static com.epam.jdi.mobile.logger.AllureLoggerHelper.failStep;
import static com.epam.jdi.mobile.logger.AllureLoggerHelper.takeHtmlCodeOnFailure;
import static com.epam.jdi.mobile.logger.LogLevels.ERROR;
import static com.epam.jdi.mobile.logger.LogLevels.STEP;
...
public class ActionHelper {
    static String getTemplate(LogLevels level) {
        return level.equalOrMoreThan(STEP) ? OutputTemplates.STEP_TEMPLATE : OutputTemplates.DEFAULT_TEMPLATE;
    }
    ...
    public static JFunc2<ProceedingJoinPoint, Object, Object> AFTER_STEP_ACTION = (jp, result) -> {
        ...
        if (getJpMethod(jp).getName().equals("open"))
            WebPage.BEFORE_NEW_PAGE.execute(getPage(jp.getThis()));
        TimeoutSettings.TIMEOUT.reset();
        return result;
    };
    ...
    public static String getBeforeLogString(ProceedingJoinPoint jp) {
        ...
        String logString = jp.getThis() == null
            ? actionName
            : msgFormat(getTemplate(WebSettings.logger.getLogLevel()), map(
                $("action", actionName),
    ...
    public static void processNewPage(JoinPoint jp) {
        if (WebPage.CHECK_AFTER_OPEN == PageChecks.NONE && WebSettings.VISUAL_PAGE_STRATEGY == VisualCheckPage.NONE)
            return;

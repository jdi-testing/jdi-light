...
import com.epam.jdi.mobile.elements.complex.IList;
...
public class Line implements IList<String>, IBaseElement {
    ...
    public <D> D asData(Class<D> data) {
        ...
        for (String name : headers) {
            Field field = LinqUtils.first(fields, f -> namesEqual(WebAnnotationsUtil.getElementName(f), name));
            if (field != null)
                try {
    ...
    public <D> D asData(Class<D> data, MapArray<String, String> line) {
        ...
        for (Pair<String, String> cell : line) {
            Field field = LinqUtils.first(instance.getClass().getDeclaredFields(),
                f -> namesEqual(WebAnnotationsUtil.getElementName(f), cell.key));
            if (field != null)
                try {
    ...
    public <T> T asLine(Class<T> cl) {
        ...
        for (int i = 0; i < headers.size(); i++) {
            ...
            Field field = LinqUtils.first(instance.getClass().getDeclaredFields(),
                f -> namesEqual(WebAnnotationsUtil.getElementName(f), header));

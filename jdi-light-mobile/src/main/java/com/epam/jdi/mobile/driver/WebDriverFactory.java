...
import com.epam.jdi.mobile.driver.get.DriverData;
import com.epam.jdi.mobile.driver.get.DriverInfos;
import com.epam.jdi.mobile.driver.get.RemoteDriver;
...
public class WebDriverFactory {
    public static MapArray<String, JFunc<WebDriver>> DRIVERS
        = new MapArray<>(DriverData.DEFAULT_DRIVER, () -> initDriver(DriverTypes.CHROME));
    ...
    public static String useDriver(String driverName) {
        return useDriver(DriverTypes.getByName(driverName));
    }
    ...
    public static boolean isRemote() {
        return isNotEmpty(RemoteDriver.DRIVER_REMOTE_URL);
    }

    private static WebDriver initDriver(DriverTypes type) {
        WebDriver driver = Switch(type).get(
            Value(DriverTypes.CHROME, t -> DriverInfos.CHROME_INFO.getDriver()),
            Value(DriverTypes.FIREFOX, t -> DriverInfos.FF_INFO.getDriver()),
            Value(DriverTypes.IE, t -> DriverInfos.IE_INFO.getDriver()),
            Value(DriverTypes.OPERA, t -> DriverInfos.CHROME_INFO.getDriver()),
            Value(DriverTypes.EDGE, t -> DriverInfos.CHROME_INFO.getDriver())
        );
        if (driver == null)
            throw exception("Unknown driver: " + type);
        return DriverData.DRIVER_SETTINGS.execute(driver);
    }
    ...
    public static WebDriver getDriver() {
        try {
            if (isNotBlank(DriverData.DRIVER_NAME))
                return getDriver(DriverData.DRIVER_NAME);
            useDriver(DriverTypes.CHROME);
            return getDriver(DriverTypes.CHROME.name);
        } catch (Exception ex) {
    ...
    public static void reopenDriver() {
        reopenDriver(DriverData.DRIVER_NAME);
    }

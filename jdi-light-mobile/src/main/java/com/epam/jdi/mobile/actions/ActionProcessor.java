...
public class ActionProcessor {
    ...
    @Pointcut("execution(* *(..)) && @annotation(JDIAction)")
    ...
    public Object jdiAround(ProceedingJoinPoint jp) {
        try {
            ...
            ActionHelper.BEFORE_JDI_ACTION.execute(jp);
            Object result = stableAction(jp);
            isOverride.get().clear();
            if (aroundCount() == 1)
                getDriver().manage().timeouts().implicitlyWait(TimeoutSettings.TIMEOUT.get(), TimeUnit.SECONDS);
            return ActionHelper.AFTER_JDI_ACTION.execute(jp, result);
        } catch (Throwable ex) {
            Object element = getObjAround(jp);
            addFailedMethod(jp);
            if (aroundCount() == 1) {
                ActionHelper.logFailure(element);
                reverse(failedMethods);
                WebSettings.logger.error("Failed actions chain: " + PrintUtils.print(failedMethods, " > "));
            }
            throw exception(ex, ActionHelper.ACTION_FAILED.execute(element, getExceptionAround(ex, aroundCount() == 1)));
        }
    }
    ...
    public static ICoreElement getJdi(ProceedingJoinPoint jp) {
        try {
            return jp.getThis() != null && ReflectionUtils.isInterface(ActionHelper.getJpClass(jp), ICoreElement.class)
    ...
    static JFunc1<Object, Object> getOverride(ProceedingJoinPoint jp) {
        ...
        JFunc1<Object, Object> override = ActionOverride.getOverrideAction(jp);
        if (override != null)
    ...
    static String getFailedMessage(ProceedingJoinPoint jp, String exception) {
        ...
        try {
            String result = StringUtils.msgFormat(OutputTemplates.FAILED_ACTION_TEMPLATE, map(
                $("exception", exception),
                $("timeout", getTimeout(jp)),
                $("action", method.getMethod().getName())
            ));
            return ActionHelper.fillTemplate(result, jp, method);
        } catch (Exception ex) {
    ...
    public Object stepAround(ProceedingJoinPoint jp) {
        try {
            ActionHelper.BEFORE_STEP_ACTION.execute(jp);
            Object result = jp.proceed();
            return ActionHelper.AFTER_STEP_ACTION.execute(jp, result);
        } catch (Throwable ex) {
            Object element = jp.getThis() != null ? jp.getThis() : new Object();
            throw exception(ex, ActionHelper.ACTION_FAILED.execute(element, safeException(ex)));
        }
    }
}

...
import com.epam.jdi.mobile.elements.complex.*;
import com.epam.jdi.mobile.elements.init.entities.collection.EntitiesCollection;
import com.epam.jdi.mobile.elements.pageobjects.annotations.*;
import com.epam.jdi.mobile.elements.pageobjects.annotations.locators.*;
...
public class InitActions {
    ...
    static void webPageSetup(SiteInfo info) {
        ...
        page.updatePageData(
            valueOrDefault(WebAnnotationsUtil.getAnnotation(info.field, Url.class),
                page.getClass().getAnnotation(Url.class)),
            valueOrDefault(WebAnnotationsUtil.getAnnotation(info.field, Title.class),
                page.getClass().getAnnotation(Title.class))
        );
        EntitiesCollection.updatePage(page);
    }
    ...
    public static MapArray<String, InitRule> INIT_RULES = map(
        $("WebList", InitRule.iRule(f -> isList(f, WebElement.class), info -> new WebList())),
        $("DataList", InitRule.iRule(f -> isList(f, InitActions::isPageObject),
            info -> new DataList())),
        $("JList", InitRule.iRule(f -> f.getType() == List.class && isInterface(getGenericType(f), ICoreElement.class),
            info -> new JList())),
        $("Interface", InitRule.iRule(f -> INTERFACES.keys().contains(f.getType()),
            info -> create(INTERFACES.get(info.field.getType()))))
    );

    public static MapArray<String, SetupRule> SETUP_RULES = map(
        $("Element", SetupRule.sRule(info -> ReflectionUtils.isInterface(info.instance.getClass(), IBaseElement.class),
            InitActions::elementSetup)),
        $("ISetup", SetupRule.sRule(InitActions::isSetupValue, info -> ((ISetup)info.instance).setup(info.field))),
        $("Page", SetupRule.sRule(info -> ReflectionUtils.isClass(info.instance.getClass(), WebPage.class), InitActions::webPageSetup)),
        $("PageObject", SetupRule.sRule(info -> !isClassOr(info.type(), WebPage.class, Section.class) &&
                isPageObject(info.instance.getClass()),
            PageFactory::initElements)),
        $("VisualCheck", SetupRule.sRule(
            info -> WebSettings.VISUAL_ACTION_STRATEGY == IS_DISPLAYED && ReflectionUtils.isInterface(info.instance.getClass(), ICoreElement.class),
    ...
    public static MapArray<String, AnnotationRule> JDI_ANNOTATIONS = map(
        $("Root", AnnotationRule.aRule(Root.class, (e, a)-> e.locator.isRoot = true)),
        $("Frame", AnnotationRule.aRule(Frame.class, (e, a)-> e.setFrames(WebAnnotationsUtil.getFrames(a)))),
        $("FindBySelenium", AnnotationRule.aRule(org.openqa.selenium.support.FindBy.class,
            (e,a)-> e.setLocator(WebAnnotationsUtil.findByToBy(a)))),
        $("Css", AnnotationRule.aRule(Css.class, (e, a)-> e.setLocator(WebAnnotationsUtil.findByToBy(a)))),
        $("XPath", AnnotationRule.aRule(XPath.class, (e, a)-> e.setLocator(WebAnnotationsUtil.findByToBy(a)))),
        $("ByText", AnnotationRule.aRule(ByText.class, (e, a)-> e.setLocator(WebAnnotationsUtil.findByToBy(a)))),
        $("WithText", AnnotationRule.aRule(WithText.class, (e, a)-> e.setLocator(WebAnnotationsUtil.findByToBy(a)))),
        $("ClickArea", AnnotationRule.aRule(ClickArea.class, (e, a)-> e.clickAreaType = a.value())),
        $("GetTextAs", AnnotationRule.aRule(GetTextAs.class, (e, a)-> e.textType = a.value())),
        $("SetTextAs", AnnotationRule.aRule(SetTextAs.class, (e, a)-> e.setTextType = a.value())),
        $("NoCache", AnnotationRule.aRule(NoCache.class, (e, a)-> e.offCache())),

        $("Timeout", AnnotationRule.aRule(WaitTimeout.class, (e, a)-> e.waitSec(a.value()))),
        $("NoWait", AnnotationRule.aRule(NoWait.class, (e, a)-> e.waitSec(0))),
        $("Name", AnnotationRule.aRule(Name.class, (e, a)-> e.setName(a.value()))),
        $("GetAny", AnnotationRule.aRule(GetAny.class, (e, a)-> e.noValidation())),
        $("GetVisible", AnnotationRule.aRule(GetVisible.class, (e, a)-> e.searchVisible())),
        $("GetVisibleEnabled", AnnotationRule.aRule(GetVisibleEnabled.class, (e, a)-> e.visibleEnabled())),
        $("GetShowInView", AnnotationRule.aRule(GetShowInView.class, (e, a)-> e.inView())),
        $("Page", AnnotationRule.aRule(PageName.class, (e, a)-> e.setPage(a.value()))),

        $("ListUI", AnnotationRule.aRule(UI.class, (e, a, f)-> {
            UI[] uis = f.getAnnotationsByType(UI.class);
            if (uis.length > 0 && any(uis, j -> j.group().equals("") || j.group().equals(WebSettings.TEST_GROUP)))
                e.setLocator(WebAnnotationsUtil.findByToBy(first(uis, j -> j.group().equals(WebSettings.TEST_GROUP))));
            })),
        $("FindByUI", AnnotationRule.aRule(FindBy.class, (e, a, f)-> {
            FindBy[] jfindbys = f.getAnnotationsByType(FindBy.class);
            if (jfindbys.length > 0 && any(jfindbys, j -> j.group().equals("") || j.group().equals(WebSettings.TEST_GROUP)))
                e.setLocator(WebAnnotationsUtil.findByToBy(first(jfindbys, j -> j.group().equals(WebSettings.TEST_GROUP))));
            })),
        $("VisualCheck", AnnotationRule.aRule(VisualCheck.class, (e, a) ->  {
            if (a.value())
        ...
        if (info.field != null) {
            for (Pair<String, AnnotationRule> aRule : JDI_ANNOTATIONS) {
                try {
                    ...
                    if (WebAnnotationsUtil.hasAnnotation(info.field, annotation))

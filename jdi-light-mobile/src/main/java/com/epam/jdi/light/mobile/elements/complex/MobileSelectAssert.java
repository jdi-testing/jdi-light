...
public class MobileSelectAssert <A extends MobileSelectAssert<?,?>, E extends IMobileSelector> extends UIAssert<A, E>
    ...
    public <TEnum extends Enum<?>> MobileSelectAssert<?, ?> selected(TEnum option) {
    ...
    public MobileSelectAssert.Compare exact(int count) {
        return new MobileSelectAssert.Compare(count, this, true);
    }
    public MobileSelectAssert.Compare atLeast(int count) {
        return new MobileSelectAssert.Compare(count, this, false);
    }
    public MobileSelectAssert.Compare no() {
        return exact(0);
    }
    public MobileSelectAssert.Compare all() {
        return exact(element().size());
    }
    public MobileSelectAssert.Compare onlyOne() {
        return exact(1);
    }
    ...
    public class Compare implements JAssert {
        ...
        public String name;
        public String type;
        MobileSelectAssert<A,E> dtAssert;
        boolean exact;
        public JDIBase base() { return MobileSelectAssert.this.base(); }

        private Compare(int count,  MobileSelectAssert<A,E> dtAssert, boolean exact) {
            this.count = count;
        ...
        public MobileSelectAssert<A,E> elements(JFunc1<MobileAppUIElement, Boolean> condition) {
            jdiAssert(element().mobileList().filter(condition), hasSize(count));
            return dtAssert;
        }
        @JDIAction("Assert that '{name}' has {type} '{count}' '{0}'")
        public MobileSelectAssert<A,E> elements(MobileAppUIElement data) {
